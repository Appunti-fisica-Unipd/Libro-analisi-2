...
ax1.scatter(0, 2, marker='o', color=(0, 0, 1, 1))

ax1.scatter(-.5, 1, marker='o', color=(1, 165/255, 0, 1))
c1 = patches.Circle([-.5, 1], radius=.25, edgecolor=(1, 165/255, 0, 1), facecolor=(0, 0, 0, 0))
ax1.add_patch(c1)


with mpl.rc_context({"path.sketch":(5, 30, 1)}):
    ax1.plot([0, 0], [0, 2], color = (0, 1, 0, 1))
    ax1.plot([0, xmax], [2, 2], color = (0, 1, 0, 1))



#Add text
ax1.text(xmax + .2, 2, r"$y=f(x)$", color="b")
ax1.text(1, 1, r"esclusi da $A$", color="g")
ax1.text(-4.8, 2.4, r"""$A = \{ (x,y) \in \mathbb{R}^{2} \; \big| \; y > f(x) \}$ \\
                        $A $ aperto""", color="r")
