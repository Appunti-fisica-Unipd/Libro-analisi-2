...
def bfun(x):
    return np.array( (x - .05) * (x - .5) * (x - .95) * 2 + .5)

def rfun(x):
    return np.array( (x - .1) * (x - .6) * (x - 1.1) * 2 + .5)
...
#Add text
ax1.text(1.1, bfun(1), r"$y=f(x)$", color="b")
ax1.text(1.1, bfun(1) + .2, r"$y=f(x)+r$", color="g")
ax1.text(1.1, bfun(1) - .2, r"$y=f(x)-r$", color="y")
