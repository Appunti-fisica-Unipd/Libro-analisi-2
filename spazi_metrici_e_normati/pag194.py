...
def plot_curves(ax1): 
    ...
    ax1.plot(x, fun2(x) - .25, color = "orange", linestyle="--")
    ax1.plot(x, fun2(x) + .25, color = "orange", linestyle="--")

    ax1.plot(x, fun3(x), color="purple")
    ax1.plot(x, fun3(x) - .1, color="aqua", linestyle="--")
    ax1.plot(x, fun3(x) + .1, color="aqua", linestyle="--")
...
def fun2 (x):
    return 15 * np.array(x) * (np.array(x) - 0.75) ** 2 - .5

def fun3 (x):
    return 5 * np.array(x) * (np.array(x) - 1.25) ** 2 + .81

def plot_segments(ax1):
    ax1.plot([1,1], [-fun1(1), fun1(1)], color="g", linestyle="--")
    ax1.plot([.2,.2], [fun2(.2), fun2(.2)-.25], color="orange", linestyle="--")
...
def text(ax1):
    ...
    ax1.text(1 + dist, -fun1(1), r"$y=-\varphi(x)$", color="r")
    ax1.text(1 + dist, fun2(1), r"$y=f(x)$", color="b")
    ax1.text(0.2 + dist, fun2(.2)-.25 + 5 * dist, r"$\varepsilon$", color="orange")
