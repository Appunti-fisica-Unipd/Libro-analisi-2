...
def fun1 (x):
    return 3 / np.array(x) - 1 

def plot_segments(ax1):
    ax1.plot([1, 1], [-fun1(1) - .1, fun1(1) + .1], color="g", linestyle="--")

def plot_points(ax1):
    ax1.scatter(3, 0, color="r", marker="o")
    pass
...
def text(ax1):
    #Add text
    dist = .05
    ax1.text(1 + dist, fun1(1) + dist, r"$2$", color="r")
    ax1.text(3 + dist, fun1(3) + dist, r"$\alpha_k$", color="r")
