...
\begin{example}
	...
	Per il teorema di Bolzano-Weierstrass $\{x_k\}_{k \geq1}$ ha una sottosuccessione convergente, $\{x_{k_j}\}_{j \geq 1}$ ed un elemento $x \in \mathbb{R}$. Allora $\{x_k\}_{k \geq1}$ converge ad $x$ e quindi $\mathbb{R}$ è completo. Infatti vale la seguente.
\end{example}
\end{exbar}
...
\begin{dembar}
	Dimostrazione della Teorema \ref{th: pag231}
	...
	$$\Rightarrow |f_n(x) - \uppercomment{f_\ell(x)}{\myarrow[90]}{f(x)}|< \epsilon \qquad \forall x \in K \color{blue}{\xrightarrow{\ell \to + \infty}} |f_n(x)-f(x)| \leq \epsilon \qquad \forall n > N \text{ e } \forall x \in K$$
...
Lo stesso risultato vale in $C^0(X)_b =\{f:X \rightarrow \mathbb{R} \,\, \big| \,\, f$ è continua e limitata$\}$ con $\|f\|_\infty = \sup_{x \in X}|f(x)|$.\\
