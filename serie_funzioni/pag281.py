...
def fill(ax1):
    ax1.fill_between(x=[0,5], y1=fun([0,5]), y2=fun2([0,5]), color="b", alpha=.2)
...
if __name__ == "__main__":
    ...
    with mpl.rc_context({"path.sketch":(5, 30, 1)}):
        AX.plot([0, 3], [1, 4], color = (0, 0, 1, 1))
    text(AX, coor=[2 - .5, 4 - .2], text=r"$y=x+1$", c="r")
    text(AX, coor=[4 + .2, 4 - .2], text=r"$y=x$", c="r")
    text(AX, coor=[2 + .2, 1], text=r"""In blu l'insieme di
                                        convergenza puntuale""", c="b")
    plt.tight_layout()
    save_file()
    Dtime(START)
    plt.show()
